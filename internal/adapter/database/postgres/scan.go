...
func (s *Scanner) ScanRowToStruct(rows pgx.Rows, dest interface{}) error {
...
func (s *Scanner) ScanRowsToSlice(rows pgx.Rows, dest interface{}) error {
	destValue := reflect.ValueOf(dest)
...
func (s *Scanner) setFieldValue(field reflect.Value, val interface{}, structField reflect.StructField) error {
	...
	case "time.Time":
		...
		case string:
			// Try different time formats that PostgreSQL might return
			if parsedTime, err := time.Parse(time.RFC3339, v); err == nil {
				...
				field.Set(reflect.ValueOf(parsedTime))
			} else if parsedTime, err := time.Parse("2006-01-02 15:04:05-07", v); err == nil {
				field.Set(reflect.ValueOf(parsedTime))
			} else if parsedTime, err := time.Parse("2006-01-02 15:04:05.999999-07", v); err == nil {
				field.Set(reflect.ValueOf(parsedTime))
			} else {
				slog.Warn("Failed to parse time", "value", v, "error", err)
			}
		}
	}
