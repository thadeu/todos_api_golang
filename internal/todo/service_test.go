...
func (s *TodoServiceTestSuite) TestService_GetAllUsers_WithData() {
	...
	item1 := NewTodo[Todo](map[string]any{
		"UserId": user.ID,
	})

	s.setup.Repo.Create(context.Background(), item1)

	item2 := NewTodo[Todo](map[string]any{
		"UserId": user.ID,
	})
...
func (s *TodoServiceTestSuite) TestService_CreateTodo_Success() {
	user, _ := s.UserRepo.CreateUser(context.Background(), NewUser[User]())

	data, _ := s.setup.Repo.Create(context.Background(), NewTodo[Todo](map[string]any{
		"Title":  "Test Todo",
		"UserId": user.ID,
	}))
